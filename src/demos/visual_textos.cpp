...
int demo_visual_textos() {
    ...
    int altura_linea_abajo = static_cast<int>(bounds.top + bounds.height);
    ...
    auto ancho_frase = bounds.left + bounds.width;
    auto alto_frase_dos_lineas = bounds.top + bounds.height;

    // Altura con margenes nos da la altura de una linea incluyendo sus margenes
    // superior e inferior
    auto altura_con_margenes = alto_frase_dos_lineas - altura_linea_abajo;
    ...
    sf::Vector2f tamano_con_margenes_verticales(
        static_cast<float>(ancho_frase), static_cast<float>(altura_con_margenes)
    );
    ...
    bg_rect_2.setPosition(0, static_cast<float>(altura_con_margenes));
    ...
    last_rect.setPosition(
        static_cast<float>(ancho_frase),
        static_cast<float>(alto_frase_dos_lineas)
    );
